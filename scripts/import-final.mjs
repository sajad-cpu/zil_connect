...
// Admin credentials
const ADMIN_EMAIL = 'sajad@sellowpay.com';
const ADMIN_PASSWORD = 'NMQ9E2dDFUXeEiA';
  ...
  try {
    ...
    for (let i = 0; i < businesses.length; i++) {
      try {
        ...
        console.log(`✅ [${i + 1}/${businesses.length}] ${businesses[i].business_name}`);
        businessCount++;
      } catch (error) {
        console.error(`❌ ${businesses[i].business_name}: ${error.message}`);
      }
    }
    ...
    for (let i = 0; i < opportunities.length; i++) {
      try {
        ...
        console.log(`✅ [${i + 1}/${opportunities.length}] ${opportunities[i].title}`);
        oppCount++;
      } catch (error) {
        console.error(`❌ ${opportunities[i].title}: ${error.message}`);
      }
    }
    ...
    for (let i = 0; i < offers.length; i++) {
      try {
        ...
        console.log(`✅ [${i + 1}/${offers.length}] ${offers[i].title}`);
        offerCount++;
      } catch (error) {
        console.error(`❌ ${offers[i].title}: ${error.message}`);
      }
    }

    console.log('\n🎉 Import completed!');
    console.log('\n📊 Summary:');
    console.log(`   ✅ Businesses: ${businessCount}/${businesses.length}`);
    console.log(`   ✅ Opportunities: ${oppCount}/${opportunities.length}`);
    console.log(`   ✅ Offers: ${offerCount}/${offers.length}`);
    console.log(`\n🌐 View at: http://127.0.0.1:8090/_/`);

  } catch (error) {
    console.error('\n❌ Import failed:', error.message);
    if (error.status === 400) {
      console.error('💡 Hint: Check that collections exist and field names match');
    }
  }
}
